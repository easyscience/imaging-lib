...
**EasyImaging** is a cross-platform Python library compatible with
...
This section describes the simplest way to set up EasyImaging using
**Pixi**.
...
#### Setting up EasyImaging with Pixi
...
- Set the Python version for the Pixi environment (e.g., 3.13):
  ...
  ```
- Add EasyImaging to the Pixi environment from PyPI:
  ```txt
  pixi add --pypi easyimaging
  ```
- Add a Pixi task to run EasyImaging commands easily:
  ```txt
  pixi task add easyimaging "python -m easyimaging"
  ```

#### Updating Pixi and EasyImaging

- To update all packages in the Pixi environment, including EasyImaging:
  ```txt
  pixi update
  ```
...
EasyImaging is available on **PyPI (Python Package Index)** and can be
installed using `pip`. To do so, use the following command:

```txt
pip install easyimaging
```

To install a specific version of EasyImaging, e.g., 1.0.3:

```txt
pip install 'easyimaging==1.0.3'
```

To upgrade to the latest version:

```txt
pip install --upgrade easyimaging
```
...
```txt
pip install --upgrade --force-reinstall easyimaging
```

To check the installed version:

```txt
pip show easyimaging
```
...
To install EasyImaging from the `develop` branch of GitHub, for example:

```txt
pip install git+https://github.com/easyscience/imaging-lib@develop
```

To include extra dependencies (e.g., dev):

```txt
pip install 'easyimaging[dev] @ git+https://github.com/easyscience/imaging-lib@develop'
```

## How to Run Tutorials

EasyImaging includes a collection of **Jupyter Notebook examples** that
...
- Download all the EasyImaging tutorials to the `tutorials/` directory:
  ```txt
  pixi run easyimaging download-all-tutorials
  ```
...
- Download all the EasyImaging tutorials to the `tutorials/` directory:
  ```txt
  python -m easyimaging download-all-tutorials
  ```
...
- Open one of the `*.ipynb` files and select the
  `EasyImaging Python kernel` to get started.
